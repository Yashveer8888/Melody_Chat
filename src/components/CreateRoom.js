...
function CreateRoom() {
  ...
  const handleCreateRoom = async () => {
    try {
      ...
      const response = await fetch("https://melody-chat-backend.onrender.com/api/create-room", {
        method: "POST",
  ...
  const handleJoinRoom = async () => {
    try{
      ...
      const response = await fetch(`https://melody-chat-backend.onrender.com/api/join-room/${roomCode}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
